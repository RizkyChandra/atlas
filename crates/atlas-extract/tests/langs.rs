...
impl Remap {
    fn new(mut maps: Vec<(String, &'static str)>) -> Self {
        ...
        Remap { maps, basename_source_file: true }
    }
...
fn canon_set(items: &[Value], r: &Remap) -> Vec<String> {
    let mut v: Vec<String> = items.iter().map(|it| canon(it.as_object().unwrap(), r)).collect();
    v.sort();
    v
}
...
fn check(fixture: &str, src_path: &str, oracle_json: &str, my_extra: Vec<(String, &'static str)>, oracle_extra: Vec<(String, &'static str)>) {
    ...
    let fixture_json = std::fs::read_to_string(format!("{}/tests/fixtures/{oracle_json}.json", env!("CARGO_MANIFEST_DIR"))).expect("fixture");
    ...
    assert_eq!(got_nodes, want_nodes, "NODES mismatch for {oracle_json}\nmissing (oracle, not ours): {:?}\nextra (ours): {:?}", diff(&want_nodes, &got_nodes), diff(&got_nodes, &want_nodes));

    let want_edges = canon_set(oracle["edges"].as_array().unwrap(), &or_r);
    let got_edges = canon_set(&my_edges, &my_r);
    assert_eq!(got_edges, want_edges, "EDGES mismatch for {oracle_json}\nmissing (oracle, not ours): {:?}\nextra (ours): {:?}", diff(&want_edges, &got_edges), diff(&got_edges, &want_edges));
}
...
fn javascript_matches_oracle() {
    let src = format!("{}/tests/fixtures/jsmod/sample.js", env!("CARGO_MANIFEST_DIR"));
    let dir = make_id([Path::new(&src).parent().unwrap().to_string_lossy().as_ref()]);
    check("js", &src, "sample_js", vec![(dir, "DIR")], vec![("tmp_atlas_ora_js".into(), "DIR")]);
}
...
fn typescript_matches_oracle() {
    ...
    let dir = make_id([Path::new(&src).parent().unwrap().to_string_lossy().as_ref()]);
    check("ts", &src, "sample_ts", vec![(dir, "DIR")], vec![("tmp_atlas_ora_ts".into(), "DIR")]);
}
...
fn go_matches_oracle() {
    ...
    let pkg = make_id([Path::new(&src).parent().unwrap().file_name().unwrap().to_string_lossy().as_ref()]);
    check("go", &src, "sample_go", vec![(pkg, "DIR")], vec![("atlas_ora_go".into(), "DIR")]);
}
